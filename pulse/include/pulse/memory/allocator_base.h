...
[[nodiscard]] PULSE_FORCE_INLINE constexpr std::size_t alignUp(
...
[[nodiscard]] PULSE_FORCE_INLINE bool isAligned(
    const void* ptr, std::size_t alignment
) noexcept {
    return (reinterpret_cast<std::uintptr_t>(ptr) & (alignment - 1)) == 0;
}

/// Check if a value is a power of 2.
[[nodiscard]] PULSE_FORCE_INLINE constexpr bool isPowerOf2(std::size_t v) noexcept {
    return v > 0 && (v & (v - 1)) == 0;
}
...
[[nodiscard]] inline void* platformAlignedAlloc(std::size_t size, std::size_t alignment) noexcept {
...
struct AllocatorStats {
    ...
    PULSE_FORCE_INLINE void recordAllocation(std::size_t size, std::size_t waste = 0) noexcept {
        totalAllocations++;
...
public:
    ...
    [[nodiscard]] PULSE_FORCE_INLINE const AllocatorStats& getStats() const noexcept {
        return stats_;
    }
    ...
    [[nodiscard]] PULSE_FORCE_INLINE T* create(Args&&... args) noexcept {
        ...
        if (PULSE_UNLIKELY(mem == nullptr)) return nullptr;
        return new (mem) T(std::forward<Args>(args)...);
    }

    /// Destroy and deallocate an object of type T.
    template <typename T>
    PULSE_FORCE_INLINE void destroy(T* obj) noexcept {
        if (obj) {
            obj->~T();
...
class ScopedAllocatorReset {
public:
    explicit ScopedAllocatorReset(Allocator& alloc) noexcept : allocator_(alloc) {}
